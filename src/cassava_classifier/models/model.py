...
from .components import MultiDropoutLinear, AttentionWeighting, LabelSmoothingCrossEntropy
...
class CassavaLightningModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer,
            lr_lambda=lambda epoch: 1.0 / (1.0 + epoch)
        )
        return {
            'optimizer': optimizer,
    ...
    def on_train_epoch_end(self):
        if self.train_preds:
            f1 = f1_score(self.train_targets, self.train_preds, average='weighted', zero_division=0)
