# cassava_classifier/data/datamodule.py
...
class CassavaDataModule(LightningDataModule):
    def __init__(self, train_df, val_df, model_config, dataroot, batch_size=32, num_workers=4):
        super().__init__()
        self.train_df = train_df
        self.val_df = val_df
        self.model_config = model_config
        self.dataroot = dataroot  # ← Accept dataroot as param
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_dataset = CassavaDataset(
                self.train_df,
                self.dataroot,  # ← Use injected path
